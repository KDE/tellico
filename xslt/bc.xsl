<?xml version="1.0"?>
<xsl:stylesheet xmlns:bc="http://periapsis.org/bookcase/"
...
<!--
   ...
   $Id: bookcase-printing.xsl,v 1.9 2003/01/20 00:09:25 robby Exp $
...
<!-- To choose which properties for the books are printed, change the
     string inside the tokenize statement to a space separated
     list of attribute names. As of Bookcase 0.5, the available
     attributes are:
     - title (Title)
     - subtitle (Subtitle)
     - author (Author)
 ...
 <div id="headerblock">
  <div class="colltitle">
   ...
   <span class="subtitle">(sorted by
    <xsl:value-of select="bc:attributes/bc:attribute[@name = substring-after($sort-name, ':')]/@title"/>)
   </span>
  </div>
 </div>
  ...
  <xsl:if test="$show-headers">
   ...
   <tr>
    <xsl:for-each select="$columns">
     ...
     <th>
      <xsl:value-of select="$attributes/bc:attribute[@name = $column]/@title"/>
     </th>
    </xsl:for-each>
   </tr>
  </xsl:if>
     ...
     <tr class="book">
      <xsl:for-each select="$columns">
       ...
       <td class="attribute">
        ...
        <xsl:choose>
         <xsl:when test="$current[local-name() = $column]">

...
//  kdDebug() << "BCUnitEditWidget()" << endl;
  ...
  bb->addStretch();
  m_new = bb->addButton(i18n("&New Entry"), this, SLOT(slotHandleNew()));
  m_save = bb->addButton(i18n("Sa&ve Entry"), this, SLOT(slotHandleSave()));
  m_delete = bb->addButton(i18n("&Delete Entry"), this, SLOT(slotHandleDelete()));
...
#ifdef SHOW_COPY_BTN
  m_copy = bb->addButton(i18n("Duplicate Entry"), this, SLOT(slotHandleCopy()));
...
//  kdDebug() << "BCUnitEditWidget::slotReset()" << endl;
  ...
  m_save->setEnabled(false);
  m_delete->setEnabled(false);
  m_save->setText(i18n("Sa&ve Entry"));
  m_currColl = 0;
  m_currUnits.clear();;
...
// so do this instead
  ...
  // KAceleratorManager only descends one level. Why?
...
void BCUnitEditWidget::slotHandleDelete() {
  Bookcase* bookcase = static_cast<Bookcase*>(QObjectAncestor(parent(), "Bookcase"));
  bookcase->action("edit_delete")->activate();
...
void BCUnitEditWidget::setContents(const BCUnitList& list_) {
...
//  kdDebug() << "BCUnitEditWidget::slotSetContents() - " << list_.count() << " units" << endl;
  ...
  m_save->setText(i18n("Sa&ve Entries"));
  m_delete->setText(i18n("&Delete Entries"));
}

void BCUnitEditWidget::setContents(BCUnit* unit_, const QString& highlight_/*empty*/) {
  bool ok = queryModified();
  if(!ok) {
    return;
  }
...
//  m_tabs->showTab(0);
  ...
  m_delete->setText(i18n("&Delete Entry"));
  if(highlightPage > -1) {

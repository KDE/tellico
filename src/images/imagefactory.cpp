...
const Tellico::Data::Image& ImageFactory::addImageImpl(const QUrl& url_, bool quiet_, const QUrl& refer_, bool link_) {
  ...
  if(img.isNull()) {
    myDebug() << "Null image for" << url_.toDisplayString(QUrl::PreferLocalFile | QUrl::NormalizePathSegments);
    return Data::Image::null;
  }
...
bool ImageFactory::hasImageInDir(const QString& id_) {
  Q_ASSERT_X(factory, "ImageFactory::hasImageInDir", "ImageFactory is not initialized!");

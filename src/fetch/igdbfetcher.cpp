...
IGDBFetcher::~IGDBFetcher() = default;
...
void IGDBFetcher::populateEntry(Data::EntryPtr entry_, const QJsonObject& obj_) {
  ...
  for(const auto& ageRating : ageRatingList) {
    ...
    const int org = ratingMap.value(QLatin1StringView("organization")).toInt();
    const int rating = ratingMap.value(QLatin1StringView("rating_category")).toInt();
    if(org == 1) {

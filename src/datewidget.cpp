...
#else
#include <qapplication.h> // needed for QApplication::desktop()
#endif
...
#else
  bool poss = locale->dateMonthNamePossessive();
  for(int i = 1; i <= 12 ; ++i) {
    m_monthCombo->insertItem(poss ? locale->monthNamePossessive(i) : locale->monthName(i));
  }
//  year limits from QDate
// 1752 is empty string
  m_yearSpin = new SpinBox(1752, 7999, this);
#endif
  ...
  m_dateButton->setIconSet(BarIcon(QString::fromLatin1("date"), KIcon::SizeSmall));
...
#else
  m_daySpin->setMaxValue(date_.daysInMonth());
  m_daySpin->setValue(date_.day());
  m_monthCombo->setCurrentItem(date_.month()); // don't subtract 1 since there's the blank first item
...
#else
  m_daySpin->setMaxValue(date.daysInMonth());
#endif
  m_daySpin->blockSignals(false);
